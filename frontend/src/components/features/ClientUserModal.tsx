...
interface ClientUserModalProps {
  open: boolean; user: ClientUser | null; clients: Client[];
...
export default function ClientUserModal({ open, user, clients, onClose, onSaved }: ClientUserModalProps) {
  ...
  const [password, setPassword] = useState(''); const [clientId, setClientId] = useState('');
  ...
  useEffect(() => {
    if (open) {
      ...
      if (user) {
        ...
        setPhone(user.phone || ''); setPassword(''); setClientId(user.clientId);
        setStatus(user.status); setSelectedProjectIds(user.projects?.map((p) => p.projectId) || []);
      } else {
        setName(''); setUsername(''); setEmail(''); setPhone(''); setPassword('Client@123');
        setClientId(clients[0]?.id || ''); setStatus('Active'); setSelectedProjectIds([]);
      }
    }
  }, [open, user, clients]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !username.trim() || !email.trim() || !clientId) { setError('Please fill in all required fields.'); return; }
    setSaving(true); setError(null);
    const payload: any = { name: name.trim(), username: username.trim(), email: email.trim(), phone: phone.trim(), clientId, status, projectIds: selectedProjectIds };
    ...
    try {
      ...
      onSaved(isEdit ? `Client user ${name} updated.` : `Client user ${name} created successfully.`);
      onClose();
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in">
      <div className="bg-white rounded-2xl border border-studio-border shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-studio-border flex justify-between items-center bg-studio-sidebar/40">
          <div>
            <h3 className="text-[16px] font-bold text-studio-text">{isEdit ? 'Edit Client User' : 'New Client User'}</h3>
            <p className="text-[11.5px] text-studio-muted">Create external client user accounts for project visibility and reporting</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1 text-[12.5px]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelCls}>Full Name *</label><input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Samyutha Verma" className={inputCls} required /></div>
            <div><label className={labelCls}>Username *</label><input type="text" value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/\s+/g, ''))} placeholder="samyutha" className={inputCls} required /></div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelCls}>Email Address *</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="samyutha@abc.com" className={inputCls} required /></div>
            <div><label className={labelCls}>Mobile / Phone</label><input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 98765 43210" className={inputCls} /></div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>Client / Company *</label>
              <select value={clientId} disabled={isEdit} onChange={(e) => { setClientId(e.target.value); setSelectedProjectIds([]); }} className={`${inputCls} ${isEdit ? 'bg-studio-sidebar/60 opacity-80 cursor-not-allowed' : ''}`}>
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={isEdit ? 'Leave blank to keep unchanged' : 'Min 6 characters'} className={inputCls} />
          </div>

          <div className="pt-2 border-t border-studio-border/60">
            <label className={labelCls}>Permitted Project Assignments ({selectedProjectIds.length} selected)</label>
            <p className="text-[11px] text-studio-muted mb-2">Select which projects this client user is authorized to view:</p>
            <div className="border border-studio-border rounded-lg p-3 bg-studio-sidebar/20 max-h-36 overflow-y-auto space-y-1.5">
              {loadingProjects ? (
                ...
                clientProjects.map((proj) => (
                  <label key={proj.id} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-studio-hover cursor-pointer transition-colors">
                    ...
                    <span className="font-mono text-[11px] font-bold text-studio-text bg-white px-1.5 py-0.5 rounded border border-studio-border">{proj.id}</span>
