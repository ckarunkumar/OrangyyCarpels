...
export default function EmployeeFormView({ mode, employee, onBack, onSaved }: EmployeeFormViewProps) {
  ...
  return (
    <div className="w-full space-y-5 animate-in fade-in duration-200">
      ...
      <form id="employee-full-form" onSubmit={handleSubmit} className="bg-white border border-studio-border rounded-lg shadow-sm p-6 space-y-6">
        <div className="flex items-center gap-4 pb-4 border-b border-studio-border/70">
          <div className="relative group">
            <div className="w-16 h-16 rounded-full bg-studio-sidebar border-2 border-studio-border flex items-center justify-center overflow-hidden text-[16px] font-bold text-studio-muted shadow-2xs">{form.avatar ? <img src={form.avatar} alt="Profile" className="w-full h-full object-cover" /> : <span>{form.fullName ? form.fullName[0] : 'EMP'}</span>}</div>
            <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute -bottom-0.5 -right-0.5 w-6 h-6 rounded-full bg-brand-orange text-white flex items-center justify-center shadow hover:scale-105 transition-transform" title="Upload Photo"><Camera className="w-3 h-3" /></button>
        ...
        <div className="space-y-3">
          <div className="border-b border-studio-border/70 pb-1.5"><h3 className="text-[13px] font-bold text-studio-text uppercase tracking-wider">1. Profile & Gender</h3></div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-3.5">
            <div><label className="block text-[11px] font-bold text-brand-orange mb-1">Emp ID *</label><input type="text" placeholder="ODE0001" value={form.employeeId} onChange={set('employeeId')} className={`${inputCls(!!errors.employeeId)} font-mono uppercase font-semibold`} /></div>
