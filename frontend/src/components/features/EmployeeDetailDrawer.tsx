...
import { X, Mail, Phone, MapPin, Pencil, Calendar, Heart, Shield, Users, Globe, Building, Briefcase, Hash, ZoomIn, Landmark, CreditCard } from 'lucide-react';
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 z-50 h-full w-full max-w-[460px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-studio-border shrink-0">
          <div><h3 className="text-[15px] font-bold text-studio-text">Employee Profile</h3><p className="text-[11px] text-studio-muted font-mono">{employee.employeeId}</p></div>
          <div className="flex items-center gap-1.5">
            {isAdmin && (<button type="button" onClick={() => { onClose(); onEdit(employee); }} className="flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold text-brand-orange bg-orange-50 border border-brand-orange/30 rounded hover:bg-orange-100"><Pencil className="w-3 h-3" /> Edit Profile</button>)}
            <button type="button" onClick={onClose} className="w-7 h-7 rounded flex items-center justify-center text-studio-muted hover:bg-studio-bg"><X className="w-4 h-4" /></button>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div className="flex items-center gap-3.5 pb-3.5 border-b border-studio-border">
            <div onClick={() => setZoomOpen(true)} title="Click to zoom photo" className="relative group w-12 h-12 rounded-full bg-studio-sidebar flex items-center justify-center text-[16px] font-bold text-studio-muted border border-studio-border shrink-0 overflow-hidden cursor-pointer hover:border-brand-orange transition-all">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <span className="text-[10px] font-mono font-bold px-1.5 py-0.2 rounded bg-studio-sidebar border border-studio-border text-studio-text">{employee.employeeId}</span>
          ...
          <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2.5 bg-studio-sidebar/50 border border-studio-border rounded-lg p-3 text-[12px]">
              ...
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">Gender</span><span className="text-studio-text font-semibold">{employee.gender || 'Not Specified'}</span></div>
              ...
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">Location</span><span className="text-studio-text flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3 text-red-500" />{employee.location || '—'}</span></div>
          ...
          <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2.5 bg-studio-sidebar/50 border border-studio-border rounded-lg p-3 text-[12px]">
              ...
              <div className="col-span-2"><span className="text-studio-muted text-[10px] uppercase font-bold block">Blood Group</span><span className="text-brand-orange font-bold flex items-center gap-1 mt-0.5"><Heart className="w-3 h-3 text-brand-orange" />{employee.bloodGroup || '—'}</span></div>
          ...
          <div className="space-y-2.5">
            <h4 className="text-[11px] font-bold text-studio-text uppercase tracking-wider pb-1 border-b border-studio-border/70 flex items-center justify-between">
          ...
          <div className="space-y-2.5">
            <h4 className="text-[11px] font-bold text-studio-text uppercase tracking-wider pb-1 border-b border-studio-border/70 flex items-center gap-1.5">
              <Landmark className="w-3.5 h-3.5 text-brand-orange" />
              4. Bank & Payout Details
            </h4>
            <div className="grid grid-cols-2 gap-2.5 bg-studio-sidebar/50 border border-studio-border rounded-lg p-3 text-[12px]">
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">Bank Name</span><span className="text-studio-text font-semibold mt-0.5 block">{employee.bankName || '—'}</span></div>
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">Branch Name</span><span className="text-studio-text font-medium mt-0.5 block">{employee.branchName || '—'}</span></div>
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">IFSC Code</span><span className="text-studio-text font-mono font-bold text-brand-orange mt-0.5 block">{employee.ifscCode || '—'}</span></div>
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">Account Type</span><span className="text-studio-text font-medium mt-0.5 block">{employee.accountType || 'Savings'}</span></div>
              <div className="col-span-2"><span className="text-studio-muted text-[10px] uppercase font-bold block">Account Number</span><span className="text-studio-text font-mono font-bold text-[13px] mt-0.5 block">{employee.accountNumber || '—'}</span></div>
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">Account Holder</span><span className="text-studio-text font-medium mt-0.5 block">{employee.accountHolderName || employee.fullName}</span></div>
              <div><span className="text-studio-muted text-[10px] uppercase font-bold block">UPI ID</span><span className="text-studio-text font-mono text-[11px] mt-0.5 block">{employee.upiId || '—'}</span></div>
            </div>
          </div>

          {/* 5. Status & Access */}
          <div className="space-y-2.5">
            <h4 className="text-[11px] font-bold text-studio-text uppercase tracking-wider pb-1 border-b border-studio-border/70">5. Status & Access</h4>
