    ...
    <div className="w-full space-y-5 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between border-b border-studio-border pb-3">
        ...
        <div className="flex items-center gap-2.5">
          <button type="button" onClick={onBack} className="px-4 py-2 border border-studio-border rounded-lg text-[12px] font-semibold text-studio-text hover:bg-studio-sidebar transition-colors cursor-pointer shadow-2xs">Cancel</button>
          <button type="submit" form="project-full-form" disabled={saving} className="px-5 py-2 bg-brand-orange text-white rounded-lg text-[12px] font-bold hover:bg-opacity-90 shadow-sm disabled:opacity-50 transition-all cursor-pointer">{saving ? 'Saving...' : mode === 'edit' ? 'Save Changes' : 'Create Project'}</button>
        </div>
      </div>
      ...
      <form id="project-full-form" onSubmit={handleSubmit} className="bg-white border border-studio-border rounded-lg shadow-sm p-6 space-y-7">
        ...
        <div className="flex items-center justify-end gap-3 pt-5 border-t border-studio-border">
          <button type="button" onClick={onBack} className="px-4 py-2 border border-studio-border rounded-lg text-[12px] font-semibold text-studio-text hover:bg-studio-sidebar transition-colors cursor-pointer shadow-2xs">Cancel</button>
          <button type="submit" disabled={saving} className="px-5 py-2 bg-brand-orange text-white rounded-lg text-[12px] font-bold hover:bg-opacity-90 shadow-sm disabled:opacity-50 transition-all cursor-pointer">{saving ? 'Saving...' : mode === 'edit' ? 'Save Changes' : 'Create Project'}</button>
        </div>
      </form>
    </div>
  );
