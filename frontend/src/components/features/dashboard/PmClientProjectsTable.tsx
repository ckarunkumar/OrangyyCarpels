...
export interface PmProjectRowData {
  projectId: string;
  projectCode: string;
  projectName: string;
  clientId: string;
...
interface PmClientProjectsTableProps {
  projects: PmProjectRowData[];
  loading?: boolean;
}

export default function PmClientProjectsTable({
  projects,
  loading = false,
}: PmClientProjectsTableProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.03)] overflow-hidden">
      <div className="grid grid-cols-12 gap-3 px-6 py-3.5 bg-[#fafbfc] border-b border-slate-100 text-[10px] font-semibold text-slate-400 uppercase tracking-wider items-center">
        ...
        <div className="col-span-2 text-right">Total Logged Hours(HRS)</div>
      </div>
      ...
      <div className="divide-y divide-slate-100/80">
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400 text-[13px]">
            No assigned projects found for this client in the selected period.
          </div>
        ) : (
          projects.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div className="col-span-2 text-right font-medium text-[13px] text-slate-900">
                  {formatHours(p.totalLoggedHours)}
                </div>
              </div>
            );
          })
