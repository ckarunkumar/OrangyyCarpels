...
function isAllowedOrigin(origin) {
    if (!origin)
        return true; // Allow non-browser requests (e.g. curl / server-to-server / health checks)
    ...
    return allowed.some((allowedOrigin) => {
        if (originLower === allowedOrigin)
            return true;
        // Strip trailing slash if present
        if (originLower.replace(/\/$/, '') === allowedOrigin.replace(/\/$/, ''))
            return true;
        return false;
    });
}
// Security Headers Hook
function applySecurityHeaders(request, reply) {
