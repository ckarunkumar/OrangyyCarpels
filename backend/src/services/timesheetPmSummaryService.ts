...
export interface PmProjectSummary {
  ...
  clientId: string; clientName: string; startDate: string; endDate: string;
  billingModel: 'T&M' | 'Resources Cost (Fix)' | 'Project Cost (Fix)'; budgetHours: number;
  loggedHours: number; hoursBurnedPercent: number; status: string; totalLoggedHours: number;
}

export interface PmClientSummary {
  clientId: string; clientName: string; billingMethod: string;
  totalProjects: number; assignedProjects: number; totalLoggedHours: number;
}

export interface PmDashboardOverview {
  tmHours: number; retainerHours: number; fixedHours: number; totalHours: number;
  activeProjectsCount: number; selectedFY: string; fromDate?: string; toDate?: string;
  hasDateFilter: boolean; clients: PmClientSummary[]; projects: PmProjectSummary[];
}

export class TimesheetPmSummaryService {
  static async getPmSummary(
    role: string,
    userId?: string,
    fy?: string,
    fromDate?: string,
    ...
    const rangeStart = fromDate ? BillingCalculator.normalizeDate(fromDate) : fyStart;
    const rangeEnd = toDate ? BillingCalculator.normalizeDate(toDate) : fyEnd;
    ...
    const emp = employees.find(
      e => e.employeeId.toLowerCase() === (userId || '').toLowerCase() ||
    ...
    const isAssigned = (p: any): boolean => {
      if (role === 'Super Admin' && !isSelfScope) return true;
      const assigned = (p.assignedEmployees || '').split(',').map((s: string) => s.trim().toLowerCase());
      if (isSelfScope) return Boolean((curEmpId && assigned.includes(curEmpId)) || (curEmpName && assigned.includes(curEmpName)));
      const mgr = (p.managerId || '').toLowerCase().trim();
      const mgrName = (p.managerName || '').toLowerCase().trim();
      if (curEmpId && (mgr === curEmpId || mgrName === curEmpName || mgrName.includes(curEmpName))) return true;
    ...
    const projectSummaries: PmProjectSummary[] = [];

    for (const p of assignedProjects) {
      if (clientIdFilter && p.clientId !== clientIdFilter) continue;
      ...
      const entriesHours = relevantEntries.reduce((sum: number, d: any) => sum + (d.hours || 0), 0);
      let projectHours = entriesHours;

      if (!isActiveInPeriod && entriesHours === 0) continue;
      if (hasDateFilter && projectHours === 0) continue;
      ...
      if (billingModel === 'T&M') tmHours += projectHours;
      else if (billingModel === 'Resources Cost (Fix)') retainerHours += projectHours;
      else if (billingModel === 'Project Cost (Fix)') fixedHours += projectHours;
    ...
    const clientSummaries: PmClientSummary[] = [];
    for (const c of allClients) {
      const activeStudioProjects = (c.projects || []).filter(p => {
        const pStartNorm = BillingCalculator.normalizeDate(p.startDate);
        const pEndNorm = BillingCalculator.normalizeDate(p.endDate);
        return BillingCalculator.isProjectActiveInRange(pStartNorm, pEndNorm, rangeStart, rangeEnd);
      });
      ...
      const myActiveProjectsForClient = assignedProjects.filter(p => {
        ...
        const pStartNorm = BillingCalculator.normalizeDate(p.startDate);
        const pEndNorm = BillingCalculator.normalizeDate(p.endDate);
        const isActive = BillingCalculator.isProjectActiveInRange(pStartNorm, pEndNorm, rangeStart, rangeEnd);
      ...
      for (const p of myActiveProjectsForClient) {
        const relevantEntries = isSelfScope
          ...
          : (p.dailyEntries || []);
        const sum = relevantEntries.reduce((acc: number, d: any) => acc + (d.hours || 0), 0);
        clientHours += sum > 0 ? sum : (!hasDateFilter && !isSelfScope ? (p.loggedHours || 0) : 0);
      }

      if (hasDateFilter && clientHours === 0) continue;

      const defaultBType = c.defaultBillingType || (myActiveProjectsForClient[0]?.billingType) || 'T&M';
      const billingMethod =
        defaultBType.includes('T&M') || defaultBType.includes('Hourly') ? 'T & M' :
