...
export class SystemLogService {
  static extractClientIp(headers: Record<string, string | string[] | undefined>, socketIp?: string): string {
    ...
    if (xRealIp) return Array.isArray(xRealIp) ? xRealIp[0].trim() : xRealIp.trim();
    const fallback = socketIp || '127.0.0.1';
    return fallback === '::1' || fallback === '::ffff:127.0.0.1' ? '127.0.0.1' : fallback;
  }

  static parseUserAgent(ua?: string): { os: string; browser: string; deviceType: string } {
    ...
    let os = 'Unknown OS';
    if (/iPad|iPhone|iPod/i.test(ua)) os = 'iOS';
    ...
    else if (/curl\/|PostmanRuntime|node-fetch|undici/i.test(ua)) browser = 'API Client';

    return { os, browser, deviceType };
  }

  static async lookupIpLocation(ip: string): Promise<{
    city: string; region: string; country: string; latitude: number | null; longitude: number | null; networkInfo: string;
  }> {
  ...
  static async recordLogin(input: CreateLoginLogInput): Promise<void> {
    try {
  ...
  static async getLoginLogs(query: GetLoginLogsQuery = {}) {
